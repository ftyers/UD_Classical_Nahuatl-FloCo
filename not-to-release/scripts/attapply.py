...
class ATTFST:
    ...
    def __init__(self, attfile, epsilon_symbol=u'@0@',
                 identity_symbol=u'@_IDENTITY_SYMBOL_@',
                 unknown_symbol='@_UNKNOWN_SYMBOL_@'):
        ...
        try:
            lines = [line.rstrip('\n') for line in
                     codecs.getreader('utf-8')(gzip.open(attfile),
                                               errors='replace')]
        except:
            lines = [line.rstrip('\n') for line in
                     codecs.open(attfile, "r", encoding="utf-8")]
        self.states = {}
        self.alphabet = set()
        for l in lines:
            fields = l.split('\t')
            if len(fields) > 3:
                ...
                self.states[source].add_transition(target, insym, outsym,
                                                   weight)
    ...
    def apply(self, word, dir='down', tokenizer=None, return_joined=True):
        ...
        heappush(heap, (0.0, 0, [], 0,
                        False))  # (cost, -pos, output, state, final_included), use negpos to serve as tiebreaker
        while len(heap) > 0:
            ...
            if final_included == True:
                ...
                if -negpos == len(w):
                    ...
                    if final_included == False:
                        for outsym, target, weight in self.states[
                            state].get_transitions('', dir=dir):
                            heappush(heap, (
                            cost + weight, negpos, output + [outsym], target,
                            False))
                    if self.states[state].final == True:
                        heappush(heap, (
                        cost + self.states[state].finalweight, negpos, output,
                        state, True))
                    ...
                    for ns in nextsym:
                        for outsym, target, weight in self.states[
                            state].get_transitions(ns, dir=dir):
                            if outsym == self.identity_symbol:
                                outsym = w[-negpos]
                            elif outsym == self.unknown_symbol:
                                outsym = u'?'
                            heappush(heap, (
                            cost + weight, negpos - 1, output + [outsym],
                            target, False))
                    for outsym, target, weight in self.states[
                        state].get_transitions('', dir=dir):  # Epsilons
                        heappush(heap, (
                        cost + weight, negpos, output + [outsym], target,
                        False))
